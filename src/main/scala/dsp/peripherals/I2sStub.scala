...
class I2sReceiver(channels: Int, bclkDomain: ClockDomain) extends Component {
  ...
  val area = new ClockingArea(bclkDomain) {
    val frame = Vec.fill(channels)(Reg(SInt(32 bits)) init(0))
...
class HalfBandFir(taps: Int) extends Component {
  ...
  coeffs.foreach(_ := 0)
  val delayLine = Vec.fill(taps)(Reg(SInt(32 bits)) init(0))
...
class BramBist extends Component {
  ...
  io.error := False
  assert(io.error.getBitsWidth == 1, s"BramBist error width mismatch, expected 1")
}